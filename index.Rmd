...
message("📂 Checking for zones_ready.rds...")
if (file.exists("zones_ready.rds")) {
  message("✅ zones_ready.rds found, loading...")
  zones_poly <- readRDS("zones_ready.rds")
} else {
  stop("❌ zones_ready.rds NOT FOUND in working directory")
}

message("📂 Checking for Excel file...")
if (file.exists("Output_L2.xlsx")) {
  message("✅ Excel file found, loading...")
  mstm_data <- readxl::read_excel("Output_L2.xlsx")
} else {
  stop("❌ Excel file missing: Output_L2.xlsx")
}
...
renderPlotly({
  ...
  p_density <- ggplot(density_region, aes(
    ...
    text = paste0(
      "<b>Region:</b> ", REGION,
      "<br><b>Type:</b> ", Type,
      "<br><b>Density:</b> ", round(Density, 1), " per km²"
    )
  )) +
    ...
    labs(
      title = paste0("Population & Employment Density by Region – ", input$year_choice),
      subtitle = "Densities normalized by area (per km²)",
      x = "Region",
      y = "Density (per km²)"
    ) +
  ...
  ggplotly(p_density, tooltip = "text") %>%
    layout(
      title = list(
        text = paste0("<b>Population & Employment Density by Region – ", input$year_choice, "</b>"),
        xanchor = "center", x = 0.5
      ),
      xaxis = list(title = list(text = "Density (per km²)", font = list(size = 13)), tickformat = ",f"),
...
renderPlotly({
  ...
  p_total <- ggplot(pop_sum, aes(x = REGION, y = Value, fill = Type)) +
    ...
    ) +   # ✅ Forces full comma format (no 1e+06)
    labs(
      title = "Total Population and Households by Region",
      subtitle = "Aggregated totals better reflect Maryland’s regional distribution",
      x = "Region",
      y = "Total Value"
    ) +
  ...
  ggplotly(p_total, tooltip = "text", height = 520) %>%
    layout(
      ...
      yaxis = list(
        ...
        tickformat = ",f"   # ✅ Forces full comma style in Plotly axis too
      ),
...
renderPlotly({
  ...
  p_pop_county <- ggplot(
    ...
    scale_y_continuous(labels = function(x) format(round(x, 0), big.mark = ",", scientific = FALSE)) +  # ✅ format axis with commas
    labs(
  ...
  ggplotly(p_pop_county, tooltip = "text", height = 700) %>%
    layout(
      ...
      xaxis = list(
        ...
        tickformat = ",f"   # ✅ comma format for population values
      ),
...
renderPlotly({
  ...
  p_pop_tot <- ggplot(
    ...
    coord_flip() +
    # ✅ Force comma formatting for all y-axis values
    scale_y_continuous(labels = function(x) format(round(x, 0), big.mark = ",", scientific = FALSE)) +
    labs(
  ...
  # ✅ Render interactive plot with commas in Plotly axis
...
renderPlotly({
  ...
  p_top_counties_region <- ggplot(
    ...
    coord_flip() +
    # ✅ Add comma formatting for numeric axis
    scale_y_continuous(labels = function(x) format(round(x, 0), big.mark = ",", scientific = FALSE)) +
    labs(
      title = paste0("Most Populated County in Each Maryland Region – ", input$year_choice),
  ...
  # ✅ Interactive plot with comma tick formatting
  ggplotly(p_top_counties_region, tooltip = "text") %>%
    layout(
      title = list(
        text = paste0(
          "<b>Most Populated County in Each Maryland Region – ",
          input$year_choice,
      ...
      xaxis = list(
        title = list(text = "Total Population", font = list(size = 13)),
        tickformat = ",f"   # ✅ Full comma-based number formatting
      ),
...
renderPlotly({
  ...
  p_hh_density <- ggplot(avg_hh_density, aes(
    ...
    y = Mean_HH_Density,
    fill = REGION,
    text = paste("Region:", REGION, "<br>Avg. Household Density:", round(Mean_HH_Density, 1), "per km²")
  )) +
    geom_col(width = 0.7) +
    coord_flip() +
    labs(
  ...
  ggplotly(p_hh_density, tooltip = "text") %>%
    layout(
      title = list(
        text = "<b>Average Household Density by Region</b><br><span style='font-size:12px;color:gray'>Avg. households per km²</span>",
        xanchor = "center", x = 0.5
      ),
      xaxis = list(title = list(text = "Avg. Households per km²", font = list(size = 13))),
      yaxis = list(title = list(text = "Region", font = list(size = 13)), autorange = "reversed"),
...
renderPlotly({
  ...
  hh_labels <- c(
    "1" = "Single / Very Small (0–1.5 people)",
    "2" = "Small (1.5–2.5 people)",
    "3" = "Medium (2.5–3.5 people)",
    "4" = "Large (3.5–4.5 people)",
    "5" = "Very Large (4.5–5.5 people)",
...
renderPlotly({
  ...
  p_emp_density <- ggplot(emp_density_region, aes(
    ...
    text = paste(
      "<b>Region:</b>", REGION,
      "<br><b>Employment Density:</b>", round(Emp_Density, 1), "jobs per km²"
    )
  )) +
    ...
    labs(
      ...
      x = "Region",
      y = "Employment Density (jobs per km²)"
    ) +
    theme_minimal(base_size = 14) +
    theme(
      plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
  ...
  ggplotly(p_emp_density, tooltip = "text") %>%
    layout(
      title = list(
        text = "<b>Employment Density by Region</b><br><span style='font-size:12px;color:gray'>Jobs per km²</span>",
        xanchor = "center", x = 0.5
      ),
      xaxis = list(title = list(text = "Employment Density (jobs per km²)", font = list(size = 13)), tickformat = ",f"),
      yaxis = list(title = list(text = "Region", font = list(size = 13)), autorange = "reversed"),
      margin = list(l = 140, r = 50, t = 100, b = 100)
    )
})
...
renderPlotly({
  ...
  # ✅ Render interactive plot with comma format on axis ticks
  ggplotly(p_emp_tot, tooltip = "text") %>%
    layout(
      ...
      xaxis = list(
        ...
        tickformat = ",f"  # ✅ comma-formatted values (e.g., 1,000,000)
      ),
      yaxis = list(
        title = list(text = ""),
        automargin = TRUE
...
renderPlotly({
  ...
  p_emp_sector_tot <- ggplot(
    ...
    geom_col(show.legend = FALSE, alpha = 0.9) +
    coord_flip() +
    # ✅ Add comma formatting to numeric axis
    scale_y_continuous(labels = function(x) format(round(x, 0), big.mark = ",", scientific = FALSE)) +
    labs(
